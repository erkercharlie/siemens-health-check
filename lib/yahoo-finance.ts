...
export async function searchTicker(query: string): Promise<string | null> {
  for (const host of ["query1.finance.yahoo.com", "query2.finance.yahoo.com"]) {
    try {
      const url = `https://${host}/v1/finance/search?q=${encodeURIComponent(query)}&lang=en-US&region=US&quotesCount=5&newsCount=0`;
      ...
      const ticker = equity?.symbol ?? quotes[0]?.symbol ?? null;
...
export async function getQuoteSummary(ticker: string) {
  ...
  for (const host of ["query1.finance.yahoo.com", "query2.finance.yahoo.com"]) {
    try {
      ...
      const url = `https://${host}/v10/finance/quoteSummary/${ticker}?modules=${modules}${crumbParam}`;
...
export function extractFinancials(summary: Record<string, unknown> | null) {
  ...
  const incomeHistory = (
    (summary.incomeStatementHistory as { incomeStatementHistory?: unknown[] } | null)
      ?.incomeStatementHistory ?? []
  ) as Array<Record<string, { raw?: number } | string>>;
  const balanceHistory = (
    (summary.balanceSheetHistory as { balanceSheetStatements?: unknown[] } | null)
